...
COPY / /build
WORKDIR /build
RUN cd /build/cli_tools_tests/e2e/gce_ovf_import && CGO_ENABLED=0 go build -o /gce_ovf_import_test_runner
RUN chmod +x /gce_ovf_import_test_runner

# Build binaries to test
RUN cd /build/cli_tools/gce_ovf_import && CGO_ENABLED=0 go build -o /gce_ovf_import
...
ENTRYPOINT ["./gce_ovf_import_test_runner"]
